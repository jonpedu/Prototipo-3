...
## ✅ Implementação Completa - MVP v2.1 (Perfis + Ações + Persistência)

**Data Atualização**: 16 de dezembro de 2025  
**Status**: ✅ Totalmente Funcional  
**Versão**: 2.1 (Perfis, Ações e Persistência)  
...
ORBITA é um ambiente de programação visual para nanossatélites que converte grafos visuais em código MicroPython executável. A versão v2.1 agrega **perfis de hardware com pinos travados**, **catálogo de ações para atuadores**, **missões rápidas**, além da **persistência completa (.orbita)** e da biblioteca ampliada de drivers.

**Principais Inovações v2.1:**
- ✅ Perfis de hardware (ESP32 Genérico, Pion CanSat V1, CubeSat V1) com filtro de drivers e travamento de pinos
- ✅ Catálogo de ações para atuadores (LED RGB/branco, buzzer) com painel de arraste/solta e configuração no Inspector
- ✅ Persistência de missão: salvar/carregar `.orbita` (nodes, edges, perfil, assinatura de drivers) + presets rápidos
- ✅ Biblioteca de drivers expandida (sensores I2C: BME/BMP280, SHT20/31, CCS811, IMU; analógicos LDR/VBAT; atuadores buzzer/servo/SD logger; blocos de tempo: sequencer/delay)
- ✅ Sistema de parâmetros dinâmicos segue ativo (ex.: servo condicionado por temperatura/valor)
- ✅ UI: setas direcionais, deleção por teclado, múltipla seleção, edges estilizadas
...
|---------|--------|--------|-----------|
| `src/core/types.ts` | ✅ | ~320 | Interfaces para drivers, perfis, ações, regras lógicas |
| `src/core/drivers.ts` | ✅ | ~1100 | Biblioteca ampliada (sensores I2C/ADC, atuadores LED/Buzzer/Servo/SD, lógica tempo) |
| `src/core/transpiler.ts` | ✅ | ~570 | Kahn's Algorithm + templates condicionais + suporte a perfis |
| `src/core/serial.ts` | ✅ | ~400 | MockSerialBridge + RealSerialBridge (Web Serial API) |

### 🗄️ Estado (Zustand)
| Arquivo | Status | Linhas | Descrição |
|---------|--------|--------|-----------|
| `src/store/useStore.ts` | ✅ | ~600 | Store único: nodes, edges, telemetry, hardwareProfile, persistência `.orbita`, painel de ações |

### ⚙️ Configuração de Kits e Ações
| Arquivo | Status | Linhas | Descrição |
|---------|--------|--------|-----------|
| `src/config/hardware-profiles.ts` | ✅ | ~200 | Perfis (Genérico, Pion CanSat V1, CubeSat V1) com pin mapping e allowedDrivers |
| `src/config/actions.ts` | ✅ | ~200 | Catálogo de ações para LED/Buzzer |
| `src/config/mission-presets.ts` | ✅ | ~120 | Missões rápidas (pisca, beep, alerta combinado) |
...
| Arquivo | Status | Linhas | Descrição |
|---------|--------|--------|-----------|
| `src/components/layout/Toolbar.tsx` | ✅ | ~250 | Controles principais, salvar/carregar `.orbita`, presets de missão, seletor de perfil |
| `src/components/layout/Sidebar.tsx` | ✅ | ~180 | Drag & drop filtrado por perfil, busca, colapso por categoria |
| `src/components/layout/Canvas.tsx` | ✅ | ~200 | React Flow + deleção via teclado + setas direcionais + drop de ações |
| `src/components/layout/Inspector.tsx` | ✅ | ~750 | Parâmetros estáticos/dinâmicos, lógica, ações anexadas, avisos de segurança |
| `src/components/layout/ActionPanel.tsx` | ✅ | ~200 | Painel de ações para atuadores (arraste/solte) |
...
| Arquivo | Status | Descrição |
|---------|--------|-----------|
| `src/App.tsx` | ✅ | Orquestra layout (Toolbar + Sidebar + Canvas + Inspector + ActionPanel + Console) |
...
### ✅ Sistema de Componentes (v2.1)
- **Sensores**: Data Generator; DHT11/22; BME/BMP280; SHT20/31; CCS811; IMU MPU9250/BMX055; LDR (ADC); VBAT (ADC).
- **Atuadores**: LED branco/RGB (presets e blink), Buzzer (beep/padrão/alerta), Servo Motor (ângulo direto ou condicionado), Logger SD (append), Console Log.
- **Lógica/Tempo**: Comparador, Limiar, Delay Trigger (gatilho com atraso), Sequencer Timer (até 4 passos, repetir/loop).
- **Perfis de Hardware**: selecionador na Toolbar; pinos travados e drivers filtrados conforme kit.
- **Ações**: catálogo drag & drop para LED/Buzzer com configuração no Inspector.
