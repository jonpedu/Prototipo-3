...
`.trim()
    ...
    led_output: {
        ...
        parameters: [
            ...
            { id: 'brightness_pct', label: 'Brilho (%)', type: 'number', default: 100, min: 0, max: 100 },
            {
                id: 'input_mode',
...
{{#if input_temperature}}
    {{#if temp_operator}}
        sensor_on = {{input_temperature}} {{temp_operator}} {{temp_threshold}}
        has_input = True
        has_sensors = True
    {{/if}}
{{/if}}
...
{{#if input_humidity}}
    {{#if hum_operator}}
        sensor_on = sensor_on or ({{input_humidity}} {{hum_operator}} {{hum_threshold}})
        has_input = True
        has_sensors = True
    {{/if}}
{{/if}}
...
{{#if input_value}}
    {{#if value_operator}}
        sensor_on = sensor_on or ({{input_value}} {{value_operator}} {{value_threshold}})
        has_input = True
        has_sensors = True
    {{/if}}
{{/if}}
...
if {{led_type}} == "white":
    ...
    brightness = max(0, min(int({{brightness_pct}}), 100)) / 100.0
    r_val = int(r_val * brightness)
    g_val = int(g_val * brightness)
    b_val = int(b_val * brightness)

    {{var_name}}_pwm_r.duty(r_val)
    {{var_name}}_pwm_g.duty(g_val)
    {{var_name}}_pwm_b.duty(b_val)
